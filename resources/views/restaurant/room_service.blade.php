...
<div class="restaurant_page room-page-new">
<div id="restaurant-section">
 ...
 -->
                ...
                @if( $item->status == 'active')
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                        <div class="card">
                                ...
                                <div class="new-arrival-product all-product restaurant-all-products room-s-cat">
                                    ...
                                    <div class="food-items-sec">
                                        ...
                                        <div class="price-box d-flex justify-content-end align-items-center">
                                            ...
                                            <div class="text-right">
                                                            <?php $checkImg = false; ?>
                                                            <div class="img-btn" id="img-item-{{$item->id}}">
                                                                @if($item->image)
                                                                    <?php $checkImg = true; ?>
                                                                    <img class="cover-img slick-imgs" src="{{asset('uploads/'.$item->image)}}" alt="">
                                                                @endif
                                                                <button  id="order_item-{{$item->id}}"data-value="{{json_encode($item->only(['name','id','price','details','discount','discount_type','discount_to']))}}" data-itemId="{{$item->id}}" class="btn btn-xxs btn-info cart-addon">{{trans('layout.add')}}
                                                                </button>
                                                            </div>

                                                             <div class="modify-item" id="modify-item-{{$item->id}}" style="display:none;">
                                                                <div data-id="{{$item->id}}" class="minus-quantity" data-img="{{ $checkImg }}">
                                                                    <i class="fa fa-minus"></i>
                                                                </div>
                                                                <div id="item-quantity-{{$item->id}}" class="item-quantity">0</div>
    ...
    <div class="d-flex justify-content-center bottom-menu-btn">
            <button type="button" class="btn btn-primary" data-toggle="modal" data-target=".category-menu"><svg xmlns="http://www.w3.org/2000/svg" width="14.448" height="17.124" viewBox="0 0 14.448 17.124">
  <path id="Icon_metro-spoon-fork" data-name="Icon metro-spoon-fork" d="M6.745,1.928A3.509,3.509,0,0,0,3.535,5.674,3.675,3.675,0,0,0,6,9.319l-.532,8.664a1,1,0,0,0,1,1.068h.535a1,1,0,0,0,1-1.068L7.486,9.319a3.675,3.675,0,0,0,2.47-3.645A3.509,3.509,0,0,0,6.745,1.928Zm10.792,0-.892,5.351h-.669L15.53,1.928h-.446l-.446,5.351H13.97l-.892-5.351h-.446V8.885a.535.535,0,0,0,.535.535h1.394l-.525,8.564a1,1,0,0,0,1,1.068h.535a1,1,0,0,0,1-1.068L16.054,9.42h1.394a.535.535,0,0,0,.535-.535V1.928Z" transform="translate(-3.535 -1.928)" fill="#fff"/>
</svg>Menu</button>

            <div class="modal fade category-menu" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
              <div class="modal-dialog modal-sm">
                <div class="modal-content">
                    ...
                    <ul class="list-group">
                                @foreach($rcategories as $categoryName=>$categoryItems)
                               @if($categoryItems[0]->category->status == 'active')
                                <li class="list-group-item category-menu-item" data-id="category-name-{{ strtolower($categoryName) }}">{{ $categoryName }}</li>
                           @endif
                           @endforeach
        ...
        <div class="cart-view text-center d-flex justify-content-between p-3 mb-2 bg-dark text-white" id="proceed-to-checkout" style="visibility:hidden;">
            <div class="item-count" id="item-count">0 Item</div>
...
<script !src="">
    ...
    Number.prototype.formatNumberWithCurrSymbol = function () {
        ...
        var itemCount = $('.single-item').length;
        if (itemCount <= 0 ) {
            $('#item-count').html('0 Item');
            $("#proceed-to-checkout").css('visibility', 'hidden');
        }else{
            $('#item-count').html(`${itemCount} Item`);
            $("#proceed-to-checkout").css('visibility', 'visible');
        }
    }
    ...
    $(document).on('click','.cart-addon', function(){
        var ItemId = $(this).attr('data-itemId');
        $(`#img-item-${ItemId}`).hide();
        $(`#plus-quantity-${ItemId}`).trigger('click');
        $(`#modify-item-${ItemId}`).show();
    ...
    $(document).on('click', '.category-menu-item', function(){
    ...
    $(document).on('click', '.minus-quantity', function(e) {
        ...
        if (quantity <= 0){
           ...
           $(`#single-item-${ItemId}`).remove();
           $(`#img-item-${ItemId}`).show();
           $(`#modify-item-${ItemId}`).hide();
       }
