...
class SettingsController extends Controller
{
    public function settings()
    {
        ...
        $user = auth()->user();
        $smsGateway = RestaurantTwilioGateway::where('user_id', $user->id)->first();
        if ($smsGateway) {
            $data['sms_gateway'] = json_decode($smsGateway->value);
            $data['sms_gateway_id'] = $smsGateway->id;
            $data['sms_user_type']  = $user->type;
        }

        $vatSetting = RestaurantVatSettings::where('user_id', $user->id)->first();
        if ($vatSetting) {
            $data['vat_setting'] = json_decode($vatSetting->value);
            $data['vat_setting_id'] = $vatSetting->id;
        }

        $smsAdminGateway = Setting::where('name', 'sms_gateway')->first();
        if ($smsAdminGateway) {
            $data['sms_admin_gateway'] = json_decode($smsAdminGateway->value);
            $data['sms_admin_gateway_id'] = $smsAdminGateway->id;
        }
    ...
    public function smsGateway(Request $request)
    {
        $user = auth()->user();
        if ($user->type == 'admin') {
            $smsGateway = $request->only('twilio_sid', 'twilio_token', 'voyager_api', 'whatsappnumber','countrycode', 'voyager_api_secret', 'signalwire_project_id', 'signalware_url', 'signalware_token');
            ...
            $checkExist = RestaurantTwilioGateway::where('user_id',$user->id)->first();
            if(!empty($checkExist)){
                $smsGateway = $request->only('twilio_sid', 'twilio_token', 'whatsappnumber','countrycode','voyager_api', 'voyager_api_secret', 'signalwire_project_id', 'signalware_url', 'signalware_token',);
                $setting = isset($request->sms_gateway_id) ? RestaurantTwilioGateway::where(['id'=>$request->sms_gateway_id,'user_id'=>$user->id])->first() : new RestaurantTwilioGateway();
                $setting->user_id = $user->id;
                $setting->value = json_encode($smsGateway);
                $setting->update();
    ...
    {
        $user = auth()->user();
        $checkExist = RestaurantVatSettings::where('user_id',$user->id)->first();
        if(!empty($checkExist)){
           $vatSetting = $request->only('vat_percentage');
                $setting = isset($request->vat_setting_id) ? RestaurantVatSettings::where(['id'=>$request->vat_setting_id,'user_id'=>$user->id])->first() : new RestaurantVatSettings();
                $setting->user_id = $user->id;
                $setting->value = json_encode($vatSetting);
                $setting->update();
